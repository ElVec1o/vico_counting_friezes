...
divides both components of $\mu_iv_1-\nu_iv_0$. The choice of an initial pair is therefore
that single congruence at every index, with $(v_0,v_1)$ otherwise free subject to
$[v_0,v_1]=R$; it is not an unspecified normalisation. Whether such a pair exists for every
$R$ and every width is the one step of the correspondence left open here.
...
(Theorem~\ref{thm:transfer}). At general $R$ and general width the remaining step is the
existence of an initial pair satisfying the congruence of Theorem~\ref{thm:pathrecon}; the
criterion itself is formalized, its solvability in general is not.
