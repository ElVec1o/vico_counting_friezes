...
equation, one term for each divisor $M$ of $N^2$ with $(N/\gcd(N,M))\mid M$, the term
carrying the parameters $A=\gcd(N,M)$ and $B=M\gcd(N,M)/N$, and the number of terms is the
number of square divisors of $N^3$. At a prime the sum has two terms and collapses to the
display above; for composite $N$ it does not collapse, and no single instance of Ford's
equation carries the count.
...
\[
  T(N,5)=\sum_{\substack{M\mid N^{2}\\ d\mid M}} F\Bigl(e,\ \frac{M}{d}\Bigr),
\]
and the divisors omitted from the sum contribute nothing: no solution counted by
Theorem~\ref{thm:t5red} has $d\nmid M$.
\end{theorem}
...
\begin{corollary}\label{cor:terms}
Write $N=\prod_p p^{a_p}$. The sum of Theorem~\ref{thm:fordsum} has
$\prod_p\bigl(\lfloor 3a_p/2\rfloor+1\bigr)$ terms, the number of square divisors of $N^3$
(\href{https://oeis.org/A092520}{OEIS A092520}).
\end{corollary}

\begin{proof}
For $N=p^{a}$ and $M=p^{m}$ one has $d=p^{\max(a-m,0)}$, so $d\mid M$ reads
$\lceil a/2\rceil\le m$, and $M\mid N^2$ reads $m\le 2a$. That leaves
$2a-\lceil a/2\rceil+1=\lfloor 3a/2\rfloor+1$ values of $m$. Both the divisor set and the
condition are multiplicative in $N$.
\end{proof}
